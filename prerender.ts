...
for (const url of routes) {
  const { title = template?.match(/<title>(.+)<\/title>/)?.[1], head, app } = render(url)
  ...
  console.log(
    `${colors.gray}${dist}${colors.green}${url.length > 1 ? url.replace(/\/$/, '') : `${url}\t`}`.padEnd(padding),
